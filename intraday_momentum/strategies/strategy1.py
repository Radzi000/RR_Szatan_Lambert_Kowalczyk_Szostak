"""Strategy 1 — Asymmetric Entry/Exit Intervals.

Extends Strategy 0 by decoupling entry and exit check frequencies.
Entries are checked every 30 minutes, exits every 5 minutes, allowing
faster reaction to adverse moves while maintaining disciplined entries.
"""
...
class Strategy1(BaseStrategy):
    """Intraday momentum with asymmetric entry/exit timing.
    ...
    vol_target : float
        Daily volatility target for leverage.
    entry_interval : int
    ...
    def generate_signals(
        ...
        """Run strategy 1 with asymmetric intervals.
        ...
        for _day_date, day_bars in trading_days:
            ...
            for ts, bar in day_bars.iterrows():
                ...
                # Exit logic — checked more frequently
                if position != 0 and minute % self.exit_interval == 0:
                    if position == 1 and current_price < max(upper_bound, vwap_val):
                        ...
                        position = 0
                        continue
                    if position == -1 and current_price > min(lower_bound, vwap_val):
                        signals.append(Signal(ts, 0, reason="short exit"))
                        position = 0
                        continue

                # Entry logic — checked less frequently
                if position == 0 and minute % self.entry_interval == 0:
                    leverage = self.calculate_dynamic_leverage(recent_returns)
                    if leverage == 0:
                        continue
                    if current_price > upper_bound:
                        signals.append(Signal(ts, 1, leverage, "breakout long"))
                        position = 1
                    elif current_price < lower_bound:
                        signals.append(Signal(ts, -1, leverage, "breakout short"))
                        position = -1
