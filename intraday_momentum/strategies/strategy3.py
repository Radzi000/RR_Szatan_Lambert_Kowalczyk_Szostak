"""Strategy 3 — Exit Confirmation Counter.

Extends Strategy 1 with a confirmation mechanism for exits: the exit
condition must hold for N consecutive checks before liquidating. This
filters out whipsaw exits where price briefly dips below the exit
threshold.
"""
...
class Strategy3(BaseStrategy):
    """Intraday momentum with confirmed exits.
    ...
    exit_confirmation_bars : int
        Number of consecutive exit signals required before liquidating.
    """
    ...
    def generate_signals(
        ...
        """Run strategy 3 with confirmed exits.
        ...
        for _day_date, day_bars in trading_days:
            ...
            for ts, bar in day_bars.iterrows():
                ...
                # Exit logic with confirmation counter
